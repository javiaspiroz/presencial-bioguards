...
public class ControladorRegistro {
	...
	void continuarRegistro(ActionEvent event) {	
		...
		if(miFechaDeNacimiento.getValue()!=null) {
			String fecha = miFechaDeNacimiento.getEditor().getText();
			fechaNacimiento = FuncionesAuxiliares.StringToTimestamp(fecha, "guardar");
		}
	...
	private boolean verificarDatosGridPane1(String nombre, String apellidos, String contrasena ,String dni, 
											Timestamp fechaNacimiento , String contrasenaCorrecta, int tipo ) {	
		if (!nombre.equalsIgnoreCase("") && !apellidos.equalsIgnoreCase("") && !dni.equalsIgnoreCase("") && 
			...
			contrasena.equalsIgnoreCase(contrasenaCorrecta) && FuncionesAuxiliares.validarPassword(contrasena).equalsIgnoreCase("Contraseña segura") &&
			...
			} else if(!FuncionesAuxiliares.validarPassword(contrasena).equalsIgnoreCase("Contraseña segura")) { //Verifica que la contraseña es segura
	...
	private boolean verificarDatosGridPane2(String email, int telefono, String direccion, String ciudad, 
		...
		if (FuncionesAuxiliares.validarEmail(email) && FuncionesAuxiliares.validarValorNumerico(miTelefono.getText(), 9) &&
			...
			if(!FuncionesAuxiliares.validarEmail(email)) { //Comprueba el formato del email
				FuncionesAuxiliares.getAlertaError("Error", "El email no es válido");
				...
				FuncionesAuxiliares.getAlertaError("Error", "El telÃ©fono no es vÃ¡lido");
			} else if(direccion.equalsIgnoreCase("")) { //Verifica que se introduce un texto 
				FuncionesAuxiliares.getAlertaError("Error", "La direcciÃ³n no es vÃ¡lida");
			} else if(ciudad.equalsIgnoreCase("")) {//Verifica que se introduce un texto 
				FuncionesAuxiliares.getAlertaError("Error", "La ciudad no es vÃ¡lida");
			} else if(provincia.equalsIgnoreCase("")) {//Verifica que se introduce un texto 
				FuncionesAuxiliares.getAlertaError("Error", "La provincia no es vÃ¡lida");
				...
				FuncionesAuxiliares.getAlertaError("Error", "El cÃ³digo postal no es vÃ¡lido");
			}
			return false;
		}
	}

	private void guardarAdmin(Administrador administrador) {
		Sistema.getUnico().getfListaAdministradores().add(administrador);
		FuncionesAuxiliares.serializarArrayAJsonAdministradores();
	}
	
	private void guardarUsuario (Usuario usuario) {
		Sistema.getUnico().getFListaUsuarios().add(usuario);
